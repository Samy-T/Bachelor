...
class TSN(nn.Module):
    ...
    def _prepare_base_model(self, base_model):
        ...
        elif 'effnetv2' in base_model:
            from archs.efficientnet_v2 import effnetv2_s, effnetv2_m, effnetv2_l, effnetv2_xl, MBConv
            if base_model == 'effnetv2_s':
                self.base_model = effnetv2_s()
            if base_model == 'effnetv2_m':
                self.base_model = effnetv2_m()
            if base_model == 'effnetv2_l':
                self.base_model = effnetv2_l()
            if base_model == 'effnetv2_xl':
                self.base_model = effnetv2_xl()
    ...
    def _get_diff(self, input, keep_rgb=False):
        input_c = 3 if self.modality in ["RGB"] else 2
