...
cf = confusion_matrix(video_labels, video_pred).astype(float)  # metric tool for evaluation - to evaluate the accuracy of a classification

np.save('cm.npy', cf)  # save confusion_matrix into a NumPy array file
cls_cnt = cf.sum(axis=1)  # summing values over columns from tensor (axis=0 => summing over rows | axis=1 => summing over columns)
cls_hit = np.diag(cf)  # return a tensor with diagonal elements (represents accuracy for each action categoy) of confusion_matrix

cls_acc = cls_hit / cls_cnt  # a tensor with accuracy for each action category
print(cls_acc)
upper = np.mean(np.max(cf, axis=1) / cls_cnt)  # average of action category accuracy
print('upper bound: {}'.format(upper))

print('-----Evaluation is finished------')
print('Class Accuracy {:.02f}%'.format(np.mean(cls_acc) * 100))  # Classification Accuracy - this metric is defined as the number of correct predictions divided by the total number of predictions, multiplied by 100
print('Overall Prec@1 {:.02f}% Prec@5 {:.02f}%'.format(top1.avg, top5.avg))  # Precision - this metric is defined as True_Positive / (True_Positive + False_Positive)
